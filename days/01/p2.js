//Nessa parte eu leio a emtrada e vou remotando a palavra 2 vezes, uma remontando da esqueda pra direita, e substituindo palavras por números, e outra da direita pra esquerda, também substituindo as palavras pelo número, depois junta o primeiro número encontrado na primera palavra remontada com o último da segunda e adiciona ele numa soma.
const dataO = `heightseven4two5
npskfdstpk2knsm
...
for(let p = 0; p < dataO.length; p++){
    ...
    for(let t = 0; t < word.length; t++){
        newWord += word[t]
        newWord2 = word[word.length - t - 1] + newWord2
        for(let t = 0; t < ns.length; t++){
            newWord = newWord.replace(ns[t], t+1)
            newWord2 = newWord2.replace(ns[t], t+1)
        }
    }
    data.push([newWord, newWord2])
}
const arrayMaior = []
let sum = 0
for(let i = 0; i < data.length; i++){
    ...
    for(let j = 0; j < word.length; j++){
        Number(word[j]) && array.push(word[j])
    }
    for(let j = 0; j < word2.length; j++){
        Number(word2[j]) && array2.push(word2[j])
    }
    arrayMaior.push(array[0]+''+array2[array2.length - 1])
    sum = sum + Number(arrayMaior[arrayMaior.length -1])
}
console.log(sum)