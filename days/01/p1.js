//Nesse desafio eu pego cada item da entrada e vejo se o caracter é um número, se for, adiciono ele num array e depois junto o primeiro número do array com o último, e adiciono numa soma
...
for(let i = 0; i < dataO.length; i++){
    const array = []
    for(let j = 0; j < dataO[i].length; j++){
        Number(dataO[i][j]) && array.push(dataO[i][j])
    }
    sum = sum + Number(array[0]+''+array[array.length - 1])
}
console.log(sum)