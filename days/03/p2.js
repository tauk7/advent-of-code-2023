//Nessa parte, eu crio um dicionário, onde a chave é o número da linha e o valor é outro dicionário, com a chave representando os caracteres que são números ou '*' e o valor sendo as posições que eles ocupam. Em seguida, percorro cada linha e, dentro dela, percorro seus valores. Se não for um número, verifico as posições ao redor. Se houver 2 números, multiplico ambos e adiciono o valor a um array. No final, somo todos os valores desse array.
const data = `...........................775.651...............887....79...946...921...493.....942..942.....151....155....................................
......240...................*.....-......................$..*...................*.......%.....+....................956.549.*290.......834...
...
function expandPositions(dictionary) {
  const expandedDict = {};

  for (const outerKey in dictionary) {
    expandedDict[outerKey] = {};

    for (const innerKey in dictionary[outerKey]) {
      const positions = dictionary[outerKey][innerKey];

      const expandedPositions = positions.reduce((acc, pos) => {
        const numLength = innerKey.length;
        for (let i = pos; i < pos + numLength; i++) {
          acc.push(i);
        }
        return acc;
      }, []);

      expandedDict[outerKey][innerKey] = expandedPositions;
    }
  }
...
function isValid(arr){
    ...
    arr.map(arrayN => {
        const num = arrayN[0]
        const index = arrayN.index
        dic[num] ? dic[num].push(index) : dic[num] = [index];
    })
    return dic
}
...
let dic = {}
data.split('\n').map((line, index) => {
    const arr = getAll(line)
     dic[index]= isValid(arr)
})
dic = expandPositions(dic)
const validNumbers = []
for(l in dic){
    for(n in dic[l]){
        if(!Number(n)){
        dic[l][n].map(i => {
            const arr = []
            arr.push(i-1, i+n.length)
            let boo = []
            arr.map(number => {
           for(n2 in dic[Number(l)]){
                    if(Number(n2) && dic[Number(l)][n2].includes(number) && !boo.includes(n2)) boo.push(n2)
                }
            })
            arr.push(i)
            arr.map(number => {
                if(l !== 0) for(n2 in dic[Number(l)-1]){
                    if(Number(n2) && dic[Number(l) - 1][n2].includes(number) && !boo.includes(n2)) boo.push(n2)
                    }
                    
                if(l !== Object.keys(l).length) for(n2 in dic[Number(l)+1]){
                        if(Number(n2)){
                            if(Number(n2) && dic[Number(l) + 1][n2].includes(number) && !boo.includes(n2)) boo.push(n2)
                    }
                }
            })
            boo.length > 1 && validNumbers.push( boo[0]*boo[1])
        })
    }
}
}
let sum = 0
validNumbers.map(n => sum += n)
console.log(sum)